...
def processar_pedido(rest, key, row, cfg):
    ...
    try:
        ...
        return "cancelada"
    except coletor_ldi.CookieVencido as e:  # a classe vive em coletor_ldi (subclasse de SystemExit)
        _patch_pedido(rest, key, pid, {"status": "aguardando_cookie", "mensagem": str(e)[:400]})
        _publicar_cookie_status(rest, key, cookie)  # publica o status do cookie (agora inválido) p/ o banner
